...
use snowflake_rpc::{buffer::BufferId, plugin::PluginId, proxy::ProxyResponse};
use snowflake_xi_rope::{Rope, RopeDelta, Transformer};
use lsp_types::{
...
use crate::{
    command::{CommandKind, InternalCommand, SnowflakeCommand, SnowflakeWorkbenchCommand},
...
impl EditorData {
    ...
    fn run_edit_command(&self, cmd: &EditCommand) -> CommandExecuted {
        ...
        let yank_data =
            if let snowflake_core::cursor::CursorMode::Visual { .. } = &cursor.mode {
    ...
    fn run_multi_selection_command(
        ...
        // This is currently special-cased in Snowflake because floem editor does not have 'find'
        match cmd {
    ...
    fn run_move_command(
        &self,
        movement: &snowflake_core::movement::Movement,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
            ...
            FocusCommand::JumpToNextSnippetPlaceholder => {
                self.snippet.update(|snippet| {
                    if let Some(snippet_mut) = snippet.as_mut() {
                        ...
                        {
                            let mut selection =
                                snowflake_core::selection::Selection::new();
                            let region = snowflake_core::selection::SelRegion::new(
                                *start, *end, None,
                            );
    ...
    fn inline_find(&self, direction: InlineFindDirection, c: &str) {
        ...
        if let Some(new_index) = match direction {
            ...
            self.run_move_command(
                &snowflake_core::movement::Movement::Offset(
    ...
    fn on_screen_find(&self, pattern: &str) -> Vec<SelRegion> {
        ...
        if let Some((_, offset, _)) = items.first().copied() {
            self.run_move_command(
                &snowflake_core::movement::Movement::Offset(offset),
                None,
                Modifiers::empty(),
            );
        }
    ...
    fn apply_completion_item(&self, item: &CompletionItem) -> anyhow::Result<()> {
        ...
        let additional_edit: Vec<_> = item
            ...
            .flatten()
            .map(|edit| {
                let selection = snowflake_core::selection::Selection::region(
                    buffer.offset_of_position(&edit.range.start),
                    buffer.offset_of_position(&edit.range.end),
                );
                (selection, edit.new_text.as_str())
            })
            .collect::<Vec<(snowflake_core::selection::Selection, &str)>>();
    ...
    pub fn completion_apply_snippet(
        ...
        let mut selection = snowflake_core::selection::Selection::new();
        let (_tab, (start, end)) = &snippet_tabs[0];
        let region = snowflake_core::selection::SelRegion::new(*start, *end, None);
        selection.add_region(region);
        cursor.set_insert(selection);
    ...
    fn search_whole_word_forward(&self, mods: Modifiers) {
        ...
        if let Some((start, _end)) = next {
            self.run_move_command(
                &snowflake_core::movement::Movement::Offset(start),
                None,
                mods,
            );
        }
    ...
    pub fn word_at_cursor(&self) -> String {
        ...
        let region = self.cursor().with_untracked(|c| match &c.mode {
            snowflake_core::cursor::CursorMode::Normal(offset) => {
                snowflake_core::selection::SelRegion::caret(*offset)
            }
            snowflake_core::cursor::CursorMode::Visual {
                start,
                end,
                mode: _,
            } => snowflake_core::selection::SelRegion::new(
                *start.min(end),
                doc.buffer.with_untracked(|buffer| {
            ...
            snowflake_core::cursor::CursorMode::Insert(selection) => {
                        ...
                        && pointer_event.modifiers.control())
                {
                    ...
                    match rs {
                        ...
                        | FindHintRs::NoMatchContinue { .. } => {
                            self.common.snowflake_command.send(SnowflakeCommand {
    ...
    fn right_click(&self, pointer_event: &PointerInputEvent) {
                ...
                .unwrap_or_default()
            {
                vec![
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::RevealInPanel,
                    )),
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::RevealInFileExplorer,
                    )),
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::SourceControlOpenActiveFileRemoteUrl,
                    )),
                    None,
                    ...
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::AddRunDebugConfig,
                    )),
                    None,
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::PaletteCommand,
                    )),
                ]
            } else {
                vec![
                    ...
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::ShowCallHierarchy,
                    )),
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::FindReferences,
                    )),
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::GoToImplementation,
                    )),
                    Some(CommandKind::Focus(FocusCommand::Rename)),
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::RunInTerminal,
                    )),
                    None,
                    Some(CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::RevealInPanel,
        ...
        if self.diff_editor_id.get_untracked().is_some() && is_file {
            cmds.push(Some(CommandKind::Workbench(
                SnowflakeWorkbenchCommand::GoToLocation,
            )));
        }
        let snowflake_command = self.common.snowflake_command;
        for cmd in cmds {
            if let Some(cmd) = cmd {
                menu = menu.entry(
...
impl KeyPressFocus for EditorData {
    ...
    fn run_command(
        &self,
        command: &crate::command::SnowflakeCommand,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
        if self.common.find.visual.get_untracked() && self.find_focus.get_untracked()
    ...
    match cmd {
        ...
        | EditCommand::DeleteForwardAndInsert => {
            let start = match deltas.first().and_then(|delta| delta.1.els.first()) {
                Some(snowflake_xi_rope::DeltaElement::Copy(_, start)) => *start,
                _ => 0,
            };

            let end = match deltas.first().and_then(|delta| delta.1.els.get(1)) {
                Some(snowflake_xi_rope::DeltaElement::Copy(end, _)) => *end,
                _ => 0,
            };
