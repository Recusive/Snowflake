...
use snowflake_core::directory::Directory;
use snowflake_proxy::plugin::wasi::find_all_volts;
use snowflake_rpc::plugin::VoltID;
...
use crate::workspace::{SnowflakeWorkspace, SnowflakeWorkspaceType};
...
static DEFAULT_CONFIG: Lazy<config::Config> = Lazy::new(SnowflakeConfig::default_config);
static DEFAULT_LAPCE_CONFIG: Lazy<SnowflakeConfig> =
    Lazy::new(SnowflakeConfig::default_snowflake_config);
...
static DEFAULT_DARK_THEME_COLOR_CONFIG: Lazy<ColorThemeConfig> = Lazy::new(|| {
    let (_, theme) =
        SnowflakeConfig::load_color_theme_from_str(DEFAULT_DARK_THEME).unwrap();
...
impl SnowflakeConfig {
    pub fn load(
        workspace: &SnowflakeWorkspace,
        disabled_volts: &[VoltID],
        extra_plugin_paths: &[PathBuf],
    ) -> Self {
        let config = Self::merge_config(workspace, None, None);
        let mut snowflake_config: SnowflakeConfig = match config.try_deserialize() {
            Ok(config) => config,
            Err(error) => {
        ...
        snowflake_config.available_color_themes =
            Self::load_color_themes(disabled_volts, extra_plugin_paths);
        snowflake_config.available_icon_themes =
            Self::load_icon_themes(disabled_volts, extra_plugin_paths);
        snowflake_config.resolve_theme(workspace);

        snowflake_config.color_theme_list = snowflake_config
            .available_color_themes
            .values()
            .map(|(name, _)| name.clone())
            .sorted()
            .collect();
        snowflake_config.color_theme_list.sort();

        snowflake_config.icon_theme_list = snowflake_config
            .available_icon_themes
            .values()
            .map(|(name, _, _)| name.clone())
            .sorted()
            .collect();
        snowflake_config.icon_theme_list.sort();

        snowflake_config.wrap_style_list = im::vector![
        ...
        snowflake_config.terminal.get_indexed_colors();
        ...
        match workspace.kind {
            SnowflakeWorkspaceType::Local => {
                if let Some(path) = workspace.path.as_ref() {
                    let path = path.join("./.snowflake/settings.toml");
                    config = config::Config::builder()
                        .add_source(config.clone())
                        .add_source(
            ...
            SnowflakeWorkspaceType::RemoteSSH(_) => {}
            #[cfg(windows)]
            SnowflakeWorkspaceType::RemoteWSL(_) => {}
        }
    ...
    fn default_snowflake_config() -> SnowflakeConfig {
        let mut default_snowflake_config: SnowflakeConfig =
        ...
        default_snowflake_config.color_theme = DEFAULT_DARK_THEME_COLOR_CONFIG.clone();
        default_snowflake_config.icon_theme = DEFAULT_ICON_THEME_ICON_CONFIG.clone();
        default_snowflake_config.resolve_colors(None);
        default_snowflake_config
    }

    fn resolve_theme(&mut self, workspace: &SnowflakeWorkspace) {
        let default_snowflake_config = DEFAULT_LAPCE_CONFIG.clone();
        ...
        .try_deserialize::<SnowflakeConfig>()
        {
            self.core = new.core;
            self.ui = new.ui;
        ...
        self.resolve_colors(Some(&default_snowflake_config));
        self.update_id();
    }
    ...
    pub fn set_color_theme(&mut self, workspace: &SnowflakeWorkspace, theme: &str) {
    ...
    /// Note that this does not save the config.
    pub fn set_icon_theme(&mut self, workspace: &SnowflakeWorkspace, theme: &str) {
        self.core.icon_theme = theme.to_string();
        self.resolve_theme(workspace);
    }

    pub fn set_modal(&mut self, _workspace: &SnowflakeWorkspace, modal: bool) {
    ...
    fn resolve_colors(&mut self, default_config: Option<&SnowflakeConfig>) {
        ...
        let fg = self.color(SnowflakeColor::EDITOR_FOREGROUND).to_rgba8();
        let bg = self.color(SnowflakeColor::EDITOR_BACKGROUND).to_rgba8();
    ...
    pub fn files_svg(&self, paths: &[&Path]) -> (String, Option<Color>) {
        ...
        if let Some(svg) = svg {
            let color = if self.icon_theme.use_editor_color.unwrap_or(false) {
                Some(self.color(SnowflakeColor::LAPCE_ICON_ACTIVE))
            } else {
                None
            };
            (svg, color)
            ...
            (
                self.ui_svg(SnowflakeIcons::FILE),
                Some(self.color(SnowflakeColor::LAPCE_ICON_ACTIVE)),
            )
        }
    }
    ...
    pub fn symbol_svg(&self, kind: &SymbolKind) -> Option<String> {
        let kind_str = match *kind {
            SymbolKind::ARRAY => SnowflakeIcons::SYMBOL_KIND_ARRAY,
            SymbolKind::BOOLEAN => SnowflakeIcons::SYMBOL_KIND_BOOLEAN,
            SymbolKind::CLASS => SnowflakeIcons::SYMBOL_KIND_CLASS,
            SymbolKind::CONSTANT => SnowflakeIcons::SYMBOL_KIND_CONSTANT,
            SymbolKind::ENUM_MEMBER => SnowflakeIcons::SYMBOL_KIND_ENUM_MEMBER,
            SymbolKind::ENUM => SnowflakeIcons::SYMBOL_KIND_ENUM,
            SymbolKind::EVENT => SnowflakeIcons::SYMBOL_KIND_EVENT,
            SymbolKind::FIELD => SnowflakeIcons::SYMBOL_KIND_FIELD,
            SymbolKind::FILE => SnowflakeIcons::SYMBOL_KIND_FILE,
            SymbolKind::INTERFACE => SnowflakeIcons::SYMBOL_KIND_INTERFACE,
            SymbolKind::KEY => SnowflakeIcons::SYMBOL_KIND_KEY,
            SymbolKind::FUNCTION => SnowflakeIcons::SYMBOL_KIND_FUNCTION,
            SymbolKind::METHOD => SnowflakeIcons::SYMBOL_KIND_METHOD,
            SymbolKind::OBJECT => SnowflakeIcons::SYMBOL_KIND_OBJECT,
            SymbolKind::NAMESPACE => SnowflakeIcons::SYMBOL_KIND_NAMESPACE,
            SymbolKind::NUMBER => SnowflakeIcons::SYMBOL_KIND_NUMBER,
            SymbolKind::OPERATOR => SnowflakeIcons::SYMBOL_KIND_OPERATOR,
            SymbolKind::TYPE_PARAMETER => SnowflakeIcons::SYMBOL_KIND_TYPE_PARAMETER,
            SymbolKind::PROPERTY => SnowflakeIcons::SYMBOL_KIND_PROPERTY,
            SymbolKind::STRING => SnowflakeIcons::SYMBOL_KIND_STRING,
            SymbolKind::STRUCT => SnowflakeIcons::SYMBOL_KIND_STRUCT,
            SymbolKind::VARIABLE => SnowflakeIcons::SYMBOL_KIND_VARIABLE,
            _ => return None,
        };
    ...
    fn terminal_get_named_color(
        ...
        let (color, alpha) = match color {
            alacritty_terminal::vte::ansi::NamedColor::Cursor => {
                (SnowflakeColor::TERMINAL_CURSOR, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Foreground => {
                (SnowflakeColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Background => {
                (SnowflakeColor::TERMINAL_BACKGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Blue => {
                (SnowflakeColor::TERMINAL_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Green => {
                (SnowflakeColor::TERMINAL_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Yellow => {
                (SnowflakeColor::TERMINAL_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Red => {
                (SnowflakeColor::TERMINAL_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::White => {
                (SnowflakeColor::TERMINAL_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Black => {
                (SnowflakeColor::TERMINAL_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Cyan => {
                (SnowflakeColor::TERMINAL_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Magenta => {
                (SnowflakeColor::TERMINAL_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlue => {
                (SnowflakeColor::TERMINAL_BRIGHT_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightGreen => {
                (SnowflakeColor::TERMINAL_BRIGHT_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightYellow => {
                (SnowflakeColor::TERMINAL_BRIGHT_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightRed => {
                (SnowflakeColor::TERMINAL_BRIGHT_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightWhite => {
                (SnowflakeColor::TERMINAL_BRIGHT_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlack => {
                (SnowflakeColor::TERMINAL_BRIGHT_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightCyan => {
                (SnowflakeColor::TERMINAL_BRIGHT_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightMagenta => {
                (SnowflakeColor::TERMINAL_BRIGHT_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightForeground => {
                (SnowflakeColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlack => {
                (SnowflakeColor::TERMINAL_BLACK, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimRed => {
                (SnowflakeColor::TERMINAL_RED, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimGreen => {
                (SnowflakeColor::TERMINAL_GREEN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimYellow => {
                (SnowflakeColor::TERMINAL_YELLOW, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlue => {
                (SnowflakeColor::TERMINAL_BLUE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimMagenta => {
                (SnowflakeColor::TERMINAL_MAGENTA, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimCyan => {
                (SnowflakeColor::TERMINAL_CYAN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimWhite => {
                (SnowflakeColor::TERMINAL_WHITE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimForeground => {
                (SnowflakeColor::TERMINAL_FOREGROUND, 0.66)
            }
        };
