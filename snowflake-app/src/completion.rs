...
use snowflake_rpc::{plugin::PluginId, proxy::ProxyRpcHandler};
use lsp_types::{
...
use crate::{config::SnowflakeConfig, editor::EditorData, snippet::Snippet};
...
impl CompletionData {
    pub fn new(cx: Scope, config: ReadSignal<Arc<SnowflakeConfig>>) -> Self {
...
fn completion_lens_text(
    ...
    // Get only the first line of text, because Snowflake does not currently support
