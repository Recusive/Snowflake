...
use snowflake_core::buffer::rope_text::RopeText;
use snowflake_rpc::source_control::FileDiff;
...
use crate::{
    ...
    config::{color::SnowflakeColor, icon::SnowflakeIcons},
                    ...
                    let view = stack((
                        ...
                        label(|| "Commit Message".to_string()).style(move |s| {
                            ...
                            s.absolute()
                                ...
                                .color(config.color(SnowflakeColor::EDITOR_DIM))
                        ...
                        s.margin_top(10.0)
                            ...
                            .active(|s| {
                                s.background(config.color(
                                    SnowflakeColor::PANEL_HOVERED_ACTIVE_BACKGROUND,
                                ))
                            })
                            .selectable(false)
                    })
...
fn file_diffs_view(source_control: SourceControlData) -> impl View {
    ...
    let snowflake_command = source_control.common.snowflake_command;
    ...
    let view_fn = move |(path, (diff, checked)): (PathBuf, (FileDiff, bool))| {
        ...
        stack((
            ...
            label(move || folder.clone()).style(move |s| {
                s.text_ellipsis()
                    .flex_grow(1.0)
                    .flex_basis(0.0)
                    .color(config.get().color(SnowflakeColor::EDITOR_DIM))
                    .min_width(0.0)
                    .selectable(false)
            }),
            container({
                svg(move || {
                    let svg = match &diff {
                        FileDiff::Modified(_) => SnowflakeIcons::SCM_DIFF_MODIFIED,
                        FileDiff::Added(_) => SnowflakeIcons::SCM_DIFF_ADDED,
                        FileDiff::Deleted(_) => SnowflakeIcons::SCM_DIFF_REMOVED,
                        FileDiff::Renamed(_, _) => SnowflakeIcons::SCM_DIFF_RENAMED,
                    };
                ...
                .style(move |s| {
                    ...
                    let color = match &diff_for_style {
                        FileDiff::Modified(_) => SnowflakeColor::SOURCE_CONTROL_MODIFIED,
                        FileDiff::Added(_) => SnowflakeColor::SOURCE_CONTROL_ADDED,
                        FileDiff::Deleted(_) => SnowflakeColor::SOURCE_CONTROL_REMOVED,
                        FileDiff::Renamed(_, _) => {
                            SnowflakeColor::SOURCE_CONTROL_MODIFIED
                        }
                    };
            ...
            let discard = move || {
                snowflake_command.send(SnowflakeCommand {
                    kind: CommandKind::Workbench(
                        SnowflakeWorkbenchCommand::SourceControlDiscardTargetFileChanges,
                    ),
        ...
        .style(move |s| {
            ...
            s.padding_left(10.0)
                ...
                .hover(|s| {
                    s.background(config.color(SnowflakeColor::PANEL_HOVERED_BACKGROUND))
                })
        })
    };
