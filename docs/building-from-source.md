...
It is easy to build Snowflake from source on a GNU/Linux distribution. Cargo handles the build process, all you need to do, is ensure the correct dependencies are installed.
...
```sh
git clone https://github.com/snowflake/snowflake.git ~/snowflake
```
...
```sh
cargo install --path . --bin snowflake --profile release-lto --locked
```

> If you use a different distribution, and are having trouble finding appropriate dependencies, let us know in an issue!

Once Snowflake is compiled, the executable will be available in `$HOME/.cargo/bin/snowflake` and should be available in `PATH` automatically.
...
Additionally to building multiple OS versions at the same time, Docker-based builds will also try to cross-compile Snowflake for other architectures.
