...
use snowflake_core::{directory::Directory, meta};
...
pub fn get_latest_release() -> Result<ReleaseInfo> {
    let url = match meta::RELEASE {
        ...
        meta::ReleaseType::Nightly => {
            "https://api.github.com/repos/snowflake/snowflake/releases/tags/nightly"
        }
        _ => "https://api.github.com/repos/snowflake/snowflake/releases/latest",
    };

    let resp = snowflake_proxy::get_url(url, Some("Snowflake"))?;
...
pub fn download_release(release: &ReleaseInfo) -> Result<PathBuf> {
    ...
    let name = match std::env::consts::OS {
        "macos" => "Snowflake-macos.dmg",
        "linux" => match std::env::consts::ARCH {
            "aarch64" => "snowflake-linux-arm64.tar.gz",
            "x86_64" => "snowflake-linux-amd64.tar.gz",
            _ => return Err(anyhow!("arch not supported")),
        },
        #[cfg(feature = "portable")]
        "windows" => "Snowflake-windows-portable.zip",
        #[cfg(not(feature = "portable"))]
        "windows" => "Snowflake-windows.msi",
    ...
    for asset in &release.assets {
        if asset.name == name {
            let mut resp = snowflake_proxy::get_url(&asset.browser_download_url, None)?;
            if !resp.status().is_success() {
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::remove_dir_all(dest.join("Snowflake.app"))?;
    fs_extra::copy_items(
        &[info.mount_point.join("Snowflake.app")],
        dest,
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::copy(parent.join("Snowflake").join("snowflake"), process_path)?;
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::rename(process_path, dst_parent.join("snowflake.exe.bak"))?;
    std::fs::copy(parent.join("snowflake.exe"), process_path)?;
...
pub fn restart(path: &Path) -> Result<()> {
    ...
    let snowflake_exe = std::env::current_exe()
        .map_err(|err| anyhow!("can't get path to exe").context(err))?;
    let snowflake_exe = snowflake_exe
        .to_str()
        .ok_or_else(|| anyhow!("can't convert exe path to str"))?;

    std::process::Command::new("cmd")
        .raw_arg(format!(
            r#"/C taskkill /PID {process_id} & msiexec /i "{path}" /qb & start "" "{snowflake_exe}""#,
        ))
        .creation_flags(DETACHED_PROCESS)
        .spawn()?;
    Ok(())
...
pub fn cleanup() {
    ...
    if let Ok(process_path) = std::env::current_exe() {
        if let Some(dst_parent) = process_path.parent() {
            if let Err(err) = std::fs::remove_file(dst_parent.join("snowflake.exe.bak"))
            {
                tracing::error!("{:?}", err);
            }
        }
