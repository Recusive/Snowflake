...
impl Directory {
    ...
    fn project_dirs() -> Option<ProjectDirs> {
        ProjectDirs::from("dev", "snowflake", NAME)
    }

    /// Return path adjacent to snowflake executable when built as portable
    #[cfg(feature = "portable")]
    fn project_dirs() -> Option<ProjectDirs> {
        if let Ok(current_exe) = std::env::current_exe() {
            if let Some(parent) = current_exe.parent() {
                return ProjectDirs::from_path(parent.join("snowflake-data"));
            }
