...
use snowflake_core::meta;
use snowflake_rpc::proxy::ProxyStatus;

use crate::{
    app::{clickable_icon, not_clickable_icon, tooltip_label, window_menu},
    command::{SnowflakeCommand, SnowflakeWorkbenchCommand, WindowCommand},
    config::{SnowflakeConfig, color::SnowflakeColor, icon::SnowflakeIcons},
    listener::Listener,
...
fn left(
    ...
    stack((
        ...
        container(svg(move || config.get().ui_svg(SnowflakeIcons::LOGO)).style(
            move |s| {
        ...
        .popout_menu(move || window_menu(snowflake_command, workbench_command))
        .style(move |s| {
            s.margin_left(4.0)
                .margin_right(6.0)
        ...
        tooltip_label(
            config,
            container(svg(move || config.get().ui_svg(SnowflakeIcons::REMOTE)).style(
                move |s| {
                    let config = config.get();
                    ...
                    s.size(size, size).color(if is_local {
                        ...
                        match proxy_status.get() {
                            Some(_) => Color::WHITE,
                            None => config.color(SnowflakeColor::LAPCE_ICON_ACTIVE),
                        }
                    })
                },
            )),
        ...
        .popout_menu(move || {
            ...
            let mut menu = Menu::new("").entry(
                MenuItem::new("Connect to SSH Host").action(move || {
                    workbench_command.send(SnowflakeWorkbenchCommand::ConnectSshHost);
                }),
            );
            if !is_local
                ...
                })
            {
                menu = menu.entry(MenuItem::new("Disconnect remote").action(
                    move || {
                        workbench_command
                            .send(SnowflakeWorkbenchCommand::DisconnectRemote);
                    },
                ));
            }
            #[cfg(windows)]
            {
                menu = menu.entry(MenuItem::new("Connect to WSL Host").action(
                    move || {
                        workbench_command
                            .send(SnowflakeWorkbenchCommand::ConnectWslHost);
                    },
                ));
            }
            menu
            ...
            let color = if is_local {
                ...
                match proxy_status.get() {
                    Some(ProxyStatus::Connected) => {
                        config.color(SnowflakeColor::LAPCE_REMOTE_CONNECTED)
                    }
                    Some(ProxyStatus::Connecting) => {
                        config.color(SnowflakeColor::LAPCE_REMOTE_CONNECTING)
                    }
                    Some(ProxyStatus::Disconnected) => {
                        config.color(SnowflakeColor::LAPCE_REMOTE_DISCONNECTED)
                    }
...
fn middle(
    ...
    let jump_backward = move || {
        clickable_icon(
            || SnowflakeIcons::LOCATION_BACKWARD,
            move || {
                workbench_command.send(SnowflakeWorkbenchCommand::JumpLocationBackward);
            },
            || false,
    ...
    let jump_forward = move || {
        clickable_icon(
            || SnowflakeIcons::LOCATION_FORWARD,
            move || {
                workbench_command.send(SnowflakeWorkbenchCommand::JumpLocationForward);
            },
            || false,
        ...
        .popout_menu(move || {
            Menu::new("")
                ...
                .entry(MenuItem::new("Open Recent Workspace").action(move || {
                    workbench_command.send(SnowflakeWorkbenchCommand::PaletteWorkspace);
                }))
        })
    };
    ...
    stack((
        ...
        container(
            stack((
                svg(move || config.get().ui_svg(SnowflakeIcons::SEARCH)).style(
                    move |s| {
                        let config = config.get();
        ...
        .on_click_stop(move |_| {
            if workspace.clone().path.is_some() {
                workbench_command.send(SnowflakeWorkbenchCommand::PaletteHelpAndFile);
            } else {
                workbench_command.send(SnowflakeWorkbenchCommand::PaletteWorkspace);
            }
        })
        ...
        stack((
            clickable_icon(
                || SnowflakeIcons::START,
                move || {
                    workbench_command.send(SnowflakeWorkbenchCommand::PaletteRunAndDebug)
                },
                || false,
                || false,
                || "Run and Debug",
    ...
    stack((
        ...
        stack((
            ...
            .popout_menu(move || {
                Menu::new("")
                    .entry(MenuItem::new("Command Palette").action(move || {
                        workbench_command.send(SnowflakeWorkbenchCommand::PaletteCommand)
                    }))
                    .separator()
                    .entry(MenuItem::new("Open Settings").action(move || {
                        workbench_command.send(SnowflakeWorkbenchCommand::OpenSettings)
                    }))
                    .entry(MenuItem::new("Open Keyboard Shortcuts").action(
                        move || {
                            workbench_command
                                .send(SnowflakeWorkbenchCommand::OpenKeyboardShortcuts)
                        },
                    ))
                    .entry(MenuItem::new("Open Theme Color Settings").action(
                        move || {
                            workbench_command
                                .send(SnowflakeWorkbenchCommand::OpenThemeColorSettings)
                        },
                    ))
                    .separator()
                    .entry(if let Some(v) = latest_version.get_untracked() {
                        if update_in_progress.get_untracked() {
                            ...
                            MenuItem::new(format!("Restart to update ({v})")).action(
                                move || {
                                    workbench_command
                                        .send(SnowflakeWorkbenchCommand::RestartToUpdate)
                                },
                            )
                        }
                    } else {
            ...
            container(label(|| "1".to_string()).style(move |s| {
                let config = config.get();
                s.font_size(10.0)
                    .color(config.color(SnowflakeColor::EDITOR_BACKGROUND))
                    .border_radius(100.0)
                    .margin_left(5.0)
                    .margin_top(10.0)
                    .background(config.color(SnowflakeColor::EDITOR_CARET))
            }))
            .style(move |s| {
...
pub fn title(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    let snowflake_command = window_tab_data.common.snowflake_command;
...
pub fn window_controls_view(
    ...
    stack((
        clickable_icon(
            || SnowflakeIcons::WINDOW_MINIMIZE,
            || {
        ...
        clickable_icon(
            move || {
                if window_maximized.get() {
                    SnowflakeIcons::WINDOW_RESTORE
                } else {
                    SnowflakeIcons::WINDOW_MAXIMIZE
                }
            },
            move || {
