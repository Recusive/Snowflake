...
use crate::{
    ...
    language::{self, SnowflakeLanguage},
        ...
        let injection_callback = |language: &InjectionLanguageMarker| {
            let language = match language {
                InjectionLanguageMarker::Name(name) => {
                    SnowflakeLanguage::from_name(name)
                }
                InjectionLanguageMarker::Filename(path) => {
                    SnowflakeLanguage::from_path_raw(path)
                }
                InjectionLanguageMarker::Shebang(id) => SnowflakeLanguage::from_name(id),
            };
            language
...
pub struct Syntax {
    pub rev: u64,
    pub language: SnowflakeLanguage,
    pub text: Rope,
...
impl Syntax {
    pub fn init(path: &Path) -> Syntax {
        let language = SnowflakeLanguage::from_path(path);
        Syntax::from_language(language)
    }

    pub fn plaintext() -> Syntax {
        Self::from_language(SnowflakeLanguage::PlainText)
    }

    pub fn from_language(language: SnowflakeLanguage) -> Syntax {
    ...
    pub fn find_enclosing_pair(&self, offset: usize) -> Option<(usize, usize)> {
        if self.language == SnowflakeLanguage::Markdown {
