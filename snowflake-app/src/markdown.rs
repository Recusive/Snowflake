...
use snowflake_core::{language::SnowflakeLanguage, syntax::Syntax};
use snowflake_xi_rope::Rope;
use lsp_types::MarkedString;
use pulldown_cmark::{CodeBlockKind, CowStr, Event, Options, Parser, Tag};
use smallvec::SmallVec;

use crate::config::{SnowflakeConfig, color::SnowflakeColor};
                        ...
                        Tag::CodeBlock(kind) => {
                            let language =
                                if let CodeBlockKind::Fenced(language) = kind {
                                    md_language_to_snowflake_language(language)
                                } else {
                                    None
                                };
            ...
            Event::Html(text) => {
                attr_list.add_span(
                    ...
                    default_attrs
                        ...
                        .color(config.color(SnowflakeColor::MARKDOWN_BLOCKQUOTE)),
                );
                current_text.push_str(&text);
                pos += text.len();
                builder_dirty = true;
...
fn attribute_for_tag<'a>(
    ...
    match tag {
        ...
        Tag::Link {
            ...
            // TODO: Link support
            Some(default_attrs.color(config.color(SnowflakeColor::EDITOR_LINK)))
        }
...
fn md_language_to_snowflake_language(lang: &str) -> Option<SnowflakeLanguage> {
    // TODO: There are many other names commonly used that should be supported
    SnowflakeLanguage::from_name(lang)
}
...
pub fn highlight_as_code(
    ...
    default_attrs: Attrs,
    language: Option<SnowflakeLanguage>,
    text: &str,
    start_offset: usize,
    config: &SnowflakeConfig,
) {
