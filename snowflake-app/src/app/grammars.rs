...
fn get_github_api(url: &str) -> Result<String> {
    let user_agent = format!("Snowflake/{}", snowflake_core::meta::VERSION);
    let resp = snowflake_proxy::get_url(url, Some(user_agent.as_str()))?;
...
pub fn find_grammar_release() -> Result<ReleaseInfo> {
    let releases: Vec<ReleaseInfo> = serde_json::from_str(&get_github_api(
        "https://api.github.com/repos/snowflake/tree-sitter-grammars/releases?per_page=100",
    ).context("Failed to retrieve releases for tree-sitter-grammars")?)?;

    use snowflake_core::meta::{RELEASE, ReleaseType, VERSION};
