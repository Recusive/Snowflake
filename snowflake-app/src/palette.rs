...
use snowflake_core::{
    buffer::rope_text::RopeText, command::FocusCommand, language::SnowflakeLanguage,
    line_ending::LineEnding, mode::Mode, movement::Movement, selection::Selection,
    syntax::Syntax,
};
use snowflake_rpc::proxy::ProxyResponse;
...
use crate::{
    command::{
        CommandExecuted, CommandKind, InternalCommand, SnowflakeCommand, WindowCommand,
    },
    db::SnowflakeDb,
    ...
    workspace::{SnowflakeWorkspace, SnowflakeWorkspaceType, SshHost},
};
...
impl PaletteData {
        ...
        let items = workspaces
            ...
            .filter_map(|w| {
                ...
                let filter_text = match &w.kind {
                    SnowflakeWorkspaceType::Local => text,
                    SnowflakeWorkspaceType::RemoteSSH(remote) => {
                        format!("[{remote}] {text}")
                    }
                    #[cfg(windows)]
                    SnowflakeWorkspaceType::RemoteWSL(remote) => {
                        format!("[{remote}] {text}")
                    }
                };
                Some(PaletteItem {
    ...
    fn get_ssh_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let SnowflakeWorkspaceType::RemoteSSH(host) = &workspace.kind {
    ...
    fn get_wsl_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let SnowflakeWorkspaceType::RemoteWSL(host) = &workspace.kind {
    ...
    fn set_run_configs(&self, content: String) {
        ...
        if configs.is_none() {
            if let Some(path) = self.workspace.path.as_ref() {
                let path = path.join(".snowflake").join("run.toml");
                self.common
                    .internal_command
    ...
    fn get_run_configs(&self) {
        if let Some(workspace) = self.common.workspace.path.as_deref() {
            let run_toml = workspace.join(".snowflake").join("run.toml");
    ...
    fn get_languages(&self) {
        let langs = SnowflakeLanguage::languages();
        let items = langs
            .iter()
        ...
        for (name, profile) in profiles.into_iter() {
            ...
            items.push_back(PaletteItem {
                content: PaletteItemContent::TerminalProfile {
                    name: name.to_owned(),
                    profile: snowflake_rpc::terminal::TerminalProfile {
                        name: name.to_owned(),
        ...
        if let Some(item) = items.get(index) {
            match &item.content {
                PaletteItemContent::PaletteHelp { cmd } => {
                    let cmd = SnowflakeCommand {
                        kind: CommandKind::Workbench(cmd.clone()),
                        data: None,
                    };

                    self.common.snowflake_command.send(cmd);
                }
                ...
                PaletteItemContent::Command { cmd } => {
                    self.common.snowflake_command.send(cmd.clone());
                }
                    ...
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: SnowflakeWorkspace {
                                kind: SnowflakeWorkspaceType::RemoteSSH(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::WslHost { host } => {
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: SnowflakeWorkspace {
                                kind: SnowflakeWorkspaceType::RemoteWSL(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::Language { name } => {
                    ...
                    if name.is_empty() || name.to_lowercase().eq("plain text") {
                        ...
                        let lang = match SnowflakeLanguage::from_name(name) {
                            Some(v) => v,
                            None => return,
                        };
                ...
                PaletteItemContent::SCMReference { name } => {
                    self.common
                        .snowflake_command
                        .send(crate::command::SnowflakeCommand {
                        kind: CommandKind::Workbench(
                            crate::command::SnowflakeWorkbenchCommand::CheckoutReference,
                        ),
            ...
            self.common.window_common.window_command.send(
                WindowCommand::SetWorkspace {
                    workspace: SnowflakeWorkspace {
                        kind: SnowflakeWorkspaceType::RemoteSSH(ssh),
                        path: None,
                        last_open: 0,
                    },
                },
