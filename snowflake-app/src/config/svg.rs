...
const SNOWFLAKE_ICONS_DIR: Dir = include_dir!("$CARGO_MANIFEST_DIR/../icons/snowflake");
...
impl SvgStore {
    fn new() -> Self {
        ...
        svgs.insert("snowflake_logo".to_string(), LOGO.to_string());
    ...
    pub fn logo_svg(&self) -> String {
        self.svgs.get("snowflake_logo").unwrap().clone()
    }

    pub fn get_default_svg(&mut self, name: &str) -> String {
        if !self.svgs.contains_key(name) {
            let file = if name == "snowflake_remote.svg" || name == "snowflake_logo.svg" {
                SNOWFLAKE_ICONS_DIR.get_file(name).unwrap()
            } else {
                CODICONS_ICONS_DIR
                    .get_file(name)
