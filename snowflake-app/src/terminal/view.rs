...
use snowflake_core::mode::Mode;
use snowflake_rpc::{proxy::ProxyRpcHandler, terminal::TermId};
...
impl TerminalView {
    ...
    fn paint_content(
        ...
        let term_bg = config.color(SnowflakeColor::TERMINAL_BACKGROUND);
        ...
        if let Some((c, x)) = line_content.cursor {
            ...
            let cursor_color = if mode == Mode::Terminal {
                if self.run_config.with_untracked(|run_config| {
                    ...
                    config.color(SnowflakeColor::TERMINAL_CURSOR)
                }
            } else {
                config.color(SnowflakeColor::EDITOR_CARET)
            };
            if self.is_focused {
    ...
    fn paint(&mut self, cx: &mut floem::context::PaintCx) {
        ...
        if let Some(error) = self.launch_error.get() {
            ...
            text_layout.set_text(
                ...
                AttrsList::new(
                    attrs.color(config.color(SnowflakeColor::EDITOR_FOREGROUND)),
                ),
                None,
            );
            cx.draw_text(
        ...
        if let Some(selection) = content.selection.as_ref() {
            ...
            for line in start_line..end_line + 1 {
                ...
                cx.fill(
                    ...
                    config.color(SnowflakeColor::EDITOR_SELECTION),
                    0.0,
                );
            }
        ...
        //                 cx.stroke(
        //                     &rect,
        //                     config.get_color(SnowflakeColor::TERMINAL_FOREGROUND),
        //                     1.0,
        //                 );
        //                 start = *m.end();
