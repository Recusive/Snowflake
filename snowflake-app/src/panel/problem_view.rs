...
pub fn problem_panel(
    ...
    PanelBuilder::new(config, position)
        .add_style(
            ...
            move |s| {
                s.border_color(config.get().color(SnowflakeColor::LAPCE_BORDER))
...
fn file_view(
    ...
    let icon = match severity {
        DiagnosticSeverity::ERROR => SnowflakeIcons::ERROR,
        _ => SnowflakeIcons::WARNING,
    };
    let icon_color = move || {
        let config = config.get();
        match severity {
            DiagnosticSeverity::ERROR => config.color(SnowflakeColor::LAPCE_ERROR),
            _ => config.color(SnowflakeColor::LAPCE_WARN),
        }
    };
...
fn related_view(
    ...
    stack((
        ...
        stack((
            svg(move || config.get().ui_svg(SnowflakeIcons::LINK)).style(move |s| {
                let config = config.get();
                let size = config.ui.icon_size() as f32;
                s.size(size, size)
