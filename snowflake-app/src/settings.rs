...
use snowflake_core::{buffer::rope_text::RopeText, mode::Mode};
use snowflake_rpc::plugin::VoltID;
use snowflake_xi_rope::Rope;
use serde::Serialize;
...
use crate::{
    ...
    config::{
        DropdownInfo, SnowflakeConfig, color::SnowflakeColor, core::CoreConfig,
        editor::EditorConfig, icon::SnowflakeIcons, terminal::TerminalConfig,
        ui::UIConfig,
    },
...
impl KeyPressFocus for SettingsData {
    fn get_mode(&self) -> snowflake_core::mode::Mode {
        Mode::Insert
    }
    ...
    fn run_command(
        &self,
        _command: &crate::command::SnowflakeCommand,
        _count: Option<usize>,
        _mods: Modifiers,
        ...
        .style(move |s| {
            ...
            s.padding_horiz(20.0)
                ...
                .apply_if(kind == current_kind.get(), |s| {
                    s.background(config.color(SnowflakeColor::PANEL_CURRENT_BACKGROUND))
                })
                .hover(|s| {
            ...
            if let Some(is_ticked) = is_ticked {
                ...
                create_effect(move |last| {
                    ...
                    if let Ok(value) = serde::Serialize::serialize(
                        ...
                        SnowflakeConfig::update_file(&kind, &field, value);
                    }
                });
                        ...
                        text("Reset")
                            ...
                            .style(move |s| {
                                ...
                                s.margin_left(10)
                                    ...
                                    .border_radius(6)
                                    .border_color(
                                        config.color(SnowflakeColor::LAPCE_BORDER),
                                    )
...
fn dropdown_view(
    ...
    stack((
        ...
        container(
            svg(move || {
                if expanded.get() {
                    config.get().ui_svg(SnowflakeIcons::CLOSE)
                } else {
                    config.get().ui_svg(SnowflakeIcons::DROPDOWN_ARROW)
                }
            })
            .style(move |s| {
                let config = config.get();
