...
impl PluginServerHandler for LspClient {
    ...
    fn handle_did_change_text_document(
        ...
        delta: snowflake_xi_rope::RopeDelta,
        text: snowflake_xi_rope::Rope,
        new_text: snowflake_xi_rope::Rope,
        change: Arc<
            Mutex<(
        ...
        thread::spawn(move || {
            ...
            loop {
                match read_message(&mut reader) {
                    ...
                    Err(_err) => {
                        core_rpc.log(
                            snowflake_rpc::core::LogLevel::Error,
                            format!("lsp server {server} stopped!"),
                            Some(format!(
                                "snowflake_proxy::plugin::lsp::{}::{}::stopped",
            ...
            loop {
                ...
                match reader.read_line(&mut line) {
                    Ok(n) => {
                        ...
                        core_rpc.log(
                            snowflake_rpc::core::LogLevel::Trace,
                            line.trim_end().to_string(),
                            Some(format!(
                                "snowflake_proxy::plugin::lsp::{}::{}::stderr",
                                volt_id_closure.author, volt_id_closure.name
                            )),
                        );
                    }
