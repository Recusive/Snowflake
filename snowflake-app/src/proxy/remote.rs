...
pub fn start_remote(
    ...
    let remote_proxy_path = match platform {
        Windows => format!(
            "%HOMEDRIVE%%HOMEPATH%\\AppData\\Local\\snowflake\\{}\\data\\proxy",
            meta::NAME
        ),
        Darwin => format!(
            "~/Library/Application\\ Support/dev.snowflake.{}/proxy",
            meta::NAME
        ),
        _ => {
    ...
    let remote_proxy_file = match platform {
        Windows => format!("{remote_proxy_path}\\snowflake.exe"),
        _ => format!("{remote_proxy_path}/snowflake"),
    };
    ...
    if !remote
        ...
        .map(|output| {
            if meta::RELEASE == ReleaseType::Debug {
                String::from_utf8_lossy(&output.stdout).starts_with("Snowflake-proxy")
            } else {
                String::from_utf8_lossy(&output.stdout).trim()
                    == format!("Snowflake-proxy {}", meta::VERSION)
            }
        })
        .unwrap_or(false)
    {
...
fn download_remote(
    ...
    let script_install = match platform {
        HostPlatform::Windows => {
            ...
            let remote_proxy_script = "${env:TEMP}\\snowflake-proxy.ps1";
    ...
    if !script_install.success() {
        ...
        if !cmd.success() {
            let proxy_filename = format!("snowflake-proxy-{platform}-{architecture}");
            ...
            let url = format!(
                "https://github.com/no9labs/snowflake/releases/download/{proxy_version}/{proxy_filename}.gz"
            );
            debug!("proxy download URI: {url}");
            let mut resp = snowflake_proxy::get_url(url, None).expect("request failed");
