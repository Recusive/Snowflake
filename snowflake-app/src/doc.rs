...
use crate::{
    command::{CommandKind, SnowflakeCommand},
    config::{SnowflakeConfig, color::SnowflakeColor},
...
pub struct DocInfo {
    pub workspace: SnowflakeWorkspace,
    pub path: PathBuf,
...
impl Doc {
    ...
    pub fn set_language(&self, language: SnowflakeLanguage) {
    ...
    fn phantom_text(
        ...
        let hints = config
            ...
            .map(|(interval, inlay_hint)| {
                let (col, affinity) = self.buffer.with_untracked(|b| {
                    let mut cursor =
                        snowflake_xi_rope::Cursor::new(b.text(), interval.start);
                ...
                PhantomText {
                    ...
                    text,
                    affinity,
                    fg: Some(config.color(SnowflakeColor::INLAY_HINT_FOREGROUND)),
                    // font_family: Some(config.editor.inlay_hint_font_family()),
                    font_size: Some(config.editor.inlay_hint_font_size()),
                    bg: Some(config.color(SnowflakeColor::INLAY_HINT_BACKGROUND)),
                    under_line: None,
                }
            });
                            ...
                            .filter_map(|(iv, diag)| {
                                    ...
                                    && diag.severity < Some(DiagnosticSeverity::HINT)
                                {
                                    let fg = {
                                        ...
                                        {
                                            SnowflakeColor::ERROR_LENS_ERROR_FOREGROUND
                                        } else if severity
                                            == DiagnosticSeverity::WARNING
                                        {
                                            SnowflakeColor::ERROR_LENS_WARNING_FOREGROUND
                                        } else {
                                            // information + hint (if we keep that) + things without a severity
                                            SnowflakeColor::ERROR_LENS_OTHER_FOREGROUND
                                        };
        ...
        let completion_text = config
            ...
            .map(|completion| PhantomText {
                ...
                fg: Some(config.color(SnowflakeColor::COMPLETION_LENS_FOREGROUND)),
        ...
        if let Some(preedit) = self
            .preedit_phantom(Some(config.color(SnowflakeColor::EDITOR_FOREGROUND)), line)
        {
            text.push(preedit)
        }
    ...
    fn apply_layout_styles(
                        ...
                        && diag.severity < Some(DiagnosticSeverity::HINT)
                    {
                        ...
                        let color_name = match diag.severity {
                            Some(DiagnosticSeverity::ERROR) => {
                                SnowflakeColor::LAPCE_ERROR
                            }
                            _ => SnowflakeColor::LAPCE_WARN,
                        };
        ...
        if let Some(max_severity) = max_severity {
            let theme_prop = if max_severity == DiagnosticSeverity::ERROR {
                SnowflakeColor::ERROR_LENS_ERROR_BACKGROUND
            } else if max_severity == DiagnosticSeverity::WARNING {
                SnowflakeColor::ERROR_LENS_WARNING_BACKGROUND
            } else {
                SnowflakeColor::ERROR_LENS_OTHER_BACKGROUND
            };
