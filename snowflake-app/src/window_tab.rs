...
use crate::{
    ...
    command::{
        CommandExecuted, CommandKind, InternalCommand, SnowflakeCommand,
        SnowflakeWorkbenchCommand, WindowCommand,
    },
    completion::{CompletionData, CompletionStatus},
    config::SnowflakeConfig,
    db::SnowflakeDb,
    debug::{DapData, SnowflakeBreakpoint, RunDebugMode, RunDebugProcess},
    doc::DocContent,
    ...
    workspace::{SnowflakeWorkspace, SnowflakeWorkspaceType, WorkspaceInfo},
};
...
pub struct CommonData {
    pub workspace: Arc<SnowflakeWorkspace>,
    pub scope: Scope,
    ...
    pub snowflake_command: Listener<SnowflakeCommand>,
    pub workbench_command: Listener<SnowflakeWorkbenchCommand>,
    ...
    pub config: ReadSignal<Arc<SnowflakeConfig>>,
    pub proxy_status: RwSignal<Option<ProxyStatus>>,
    pub mouse_hover_timer: RwSignal<TimerToken>,
    pub breakpoints: RwSignal<BTreeMap<PathBuf, BTreeMap<usize, SnowflakeBreakpoint>>>,
...
pub struct WindowTabData {
    ...
    pub proxy: ProxyData,
    pub set_config: WriteSignal<Arc<SnowflakeConfig>>,
...
impl WindowTabData {
    ...
    pub fn new(
        ...
        let config = SnowflakeConfig::load(
            &workspace,
            &all_disabled_volts,
            &window_common.extra_plugin_paths,
        );
        let snowflake_command = Listener::new_empty(cx);
                    ...
                    .map(|(path, breakpoints)| {
                        (
                            ...
                            breakpoints
                                .into_iter()
                                .map(|b| (b.line, b))
                                .collect::<BTreeMap<usize, SnowflakeBreakpoint>>(),
                        )
                    })
                    .collect(),
            );
...

        {
            ...
            window_tab_data.common.snowflake_command.listen(move |cmd| {
                window_tab_data.run_snowflake_command(cmd);
            });
        }
        ...
        if !change_plugins.is_empty() {
            ...
            if config.core.auto_reload_plugin {
                let mut plugin_metas: HashMap<
                    String,
                    snowflake_rpc::plugin::VoltMetadata,
                > = self
                    .plugin
                    .installed
                    .get_untracked()
    ...
    pub fn run_snowflake_command(&self, cmd: SnowflakeCommand) {
        match cmd.kind {
    ...
    pub fn run_workbench_command(
        &self,
        cmd: SnowflakeWorkbenchCommand,
        data: Option<Value>,
    ) {
        use SnowflakeWorkbenchCommand::*;
        match cmd {
            // ==== Modal ====
            EnableModal => {
                    ...
                    open_file(options, move |file| {
                        if let Some(mut file) = file {
                            let workspace = SnowflakeWorkspace {
                                kind: SnowflakeWorkspaceType::Local,
            ...
            InstallToPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        ...
                        arguments: vec![String::from("-e"), format!(r#"do shell script "ln -sf '{}' /usr/local/bin/snowflake" with administrator privileges"#, std::env::args().next().unwrap())],
                    }
                )
            }
            #[cfg(target_os = "macos")]
            UninstallFromPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        program: String::from("osascript"),
                        arguments: vec![String::from("-e"), String::from(r#"do shell script "rm /usr/local/bin/snowflake" with administrator privileges"#)],
                    }
                )
            }
            JumpLocationForward => {
    ...
    fn handle_core_notification(&self, rpc: &CoreNotification) {
        ...
        match rpc {
            ...
            CoreNotification::Log {
                ...
                use snowflake_rpc::core::LogLevel;
