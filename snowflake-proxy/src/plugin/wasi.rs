...
use snowflake_xi_rope::{Rope, RopeDelta};
use lsp_types::{
...
/// `plugin_dev_path` allows launching Snowflake with a plugin on your local system for testing
/// purposes.  
...
/// use std::fs::File;
/// use std::io::Write;
/// use snowflake_proxy::plugin::wasi::load_volt;
...
pub fn start_volt(
    ...
    linker.func_wrap("snowflake", "host_handle_rpc", move || {
    ...
    linker.func_wrap("snowflake", "host_handle_stderr", move || {
        if let Ok(msg) = wasi_read_string(&stderr) {
            tracing_log::log::log!(target: &format!("snowflake_proxy::plugin::wasi::{}::{}", plugin_meta.author, plugin_meta.name), tracing_log::log::Level::Debug, "{msg}");
        }
    })?;
