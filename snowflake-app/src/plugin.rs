...
use snowflake_core::{command::EditCommand, directory::Directory, mode::Mode};
use snowflake_proxy::plugin::{download_volt, volt_icon, wasi::find_all_volts};
use snowflake_rpc::{
...
impl PluginData {
    ...
    pub fn volt_installed(&self, volt: &VoltMetadata, icon: &Option<Vec<u8>>) {
        ...
        if !is_latest {
            ...
            std::thread::spawn(move || {
                let info: Option<VoltInfo> = snowflake_proxy::get_url(url, None)
                    .ok()
    ...
    fn load_icon(volt: &VoltInfo) -> Result<VoltIcon> {
        ...
        let content = match cache_content {
            ...
            None => {
                let resp = snowflake_proxy::get_url(&url, None)?;
    ...
    fn query_volts(query: &str, offset: usize) -> Result<VoltsInfo> {
        ...
        let plugins: VoltsInfo = snowflake_proxy::get_url(url, None)?.json()?;
        Ok(plugins)
    }
            ...
            label(move || control(local_version_info.clone()))
                .style(move |s| {
                    ...
                    s.margin_left(10)
                        ...
                        .color(
                            config
                                .color(SnowflakeColor::LAPCE_BUTTON_PRIMARY_FOREGROUND),
                        )
                        .background(
                            config
                                .color(SnowflakeColor::LAPCE_BUTTON_PRIMARY_BACKGROUND),
                        )
                        .hover(|s| {
                            s.cursor(CursorStyle::Pointer).background(
                                config
                                    .color(
                                        SnowflakeColor::LAPCE_BUTTON_PRIMARY_BACKGROUND,
                                    )
                                    .multiply_alpha(0.8),
                            )
                        })
                        ...
                        .disabled(|s| {
                            s.background(config.color(SnowflakeColor::EDITOR_DIM))
                        })
                        .selectable(false)
                })
                                ...
                                stack((
                                    ...
                                    web_link(
                                        ...
                                        move || {
                                            config
                                                .get()
                                                .color(SnowflakeColor::EDITOR_LINK)
                                        },
                                        internal_command,
                                    ),
                                ))
