...
pub enum CommandKind {
    Workbench(SnowflakeWorkbenchCommand),
    Edit(EditCommand),
    Move(MoveCommand),
...
pub fn snowflake_internal_commands() -> IndexMap<String, SnowflakeCommand> {
    let mut commands = IndexMap::new();

    for c in SnowflakeWorkbenchCommand::iter() {
        let command = SnowflakeCommand {
...
pub enum SnowflakeWorkbenchCommand {
    ...
    #[cfg(target_os = "macos")]
    #[strum(message = "Install Snowflake to PATH")]
    #[strum(serialize = "install_to_path")]
    InstallToPATH,

    #[cfg(target_os = "macos")]
    #[strum(message = "Uninstall Snowflake from PATH")]
