...
use snowflake_core::{encoding::offset_utf16_to_utf8, rope_text_pos::RopeTextPosition};
use snowflake_rpc::{
    RpcError,
...
impl PluginHostHandler {
    ...
    pub fn handle_notification(
        ...
        match method.as_str() {
            ...
            StartLspServer::METHOD => {
                self.core_rpc.log(
                    snowflake_rpc::core::LogLevel::Warn,
                    format!(
                        "[{}] Usage of startLspServer as a notification is deprecated.",
                        self.volt_display_name
                    ),
                    Some(format!("snowflake_proxy::plugin::psp::{}::{}::StartLspServer", self.volt_id.author, self.volt_id.name)),
                );
            ...
            LogMessage::METHOD => {
                ...
                self.catalog_rpc.core_rpc.log_message(
                    message,
                    format!(
                        "snowflake_proxy::plugin::psp::{}::{}::LogMessage",
            ...
            _ => {
                self.core_rpc.log(
                    ...
                    Some(format!(
                        "snowflake_proxy::plugin::psp::{}::{}::{method}",
                        self.volt_id.author, self.volt_id.name
                    )),
                );
            }
