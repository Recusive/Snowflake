...
use snowflake_xi_rope::find::CaseMatching;
...
use crate::{
    ...
    config::{SnowflakeConfig, color::SnowflakeColor, editor::WrapStyle, icon::SnowflakeIcons},
    debug::{DapData, SnowflakeBreakpoint},
    doc::DocContent,
...
fn editor_wrap(config: &SnowflakeConfig) -> WrapMethod {
...
pub fn editor_style(
    ...
    .set(CursorColor, config.color(SnowflakeColor::EDITOR_CARET))
    .set(SelectionColor, config.color(SnowflakeColor::EDITOR_SELECTION))
    .set(
        CurrentLineColor,
        config.color(SnowflakeColor::EDITOR_CURRENT_LINE),
    )
    .set(
        VisibleWhitespaceColor,
        config.color(SnowflakeColor::EDITOR_VISIBLE_WHITESPACE),
    )
    .set(
        IndentGuideColor,
        config.color(SnowflakeColor::EDITOR_INDENT_GUIDE),
    )
    .set(ScrollBeyondLastLine, config.editor.scroll_beyond_last_line)
    .color(config.color(SnowflakeColor::EDITOR_FOREGROUND))
    .set(TextColor, config.color(SnowflakeColor::EDITOR_FOREGROUND))
    .set(PhantomColor, config.color(SnowflakeColor::EDITOR_DIM))
    .set(PlaceholderColor, config.color(SnowflakeColor::EDITOR_DIM))
    .set(
        PreeditUnderlineColor,
        config.color(SnowflakeColor::EDITOR_FOREGROUND),
    )
...
impl EditorView {
            ...
            match section.kind {
                ...
                DiffSectionKind::Added => {
                    cx.fill(
                        ...
                        config
                            .color(SnowflakeColor::SOURCE_CONTROL_ADDED)
                            .multiply_alpha(0.2),
                        0.0,
                    );
                }
                DiffSectionKind::Removed => {
                    cx.fill(
                        ...
                        config
                            .color(SnowflakeColor::SOURCE_CONTROL_REMOVED)
                            .multiply_alpha(0.2),
                        0.0,
                    );
                }
    ...
    fn paint_diff_no_code(
            ...
            .step_by(8)
        {
            ...
            if height > 0.0 {
                ...
                cx.stroke(
                    &Line::new(p0, p1),
                    config.color(SnowflakeColor::EDITOR_DIM),
                    &Stroke::new(1.0),
                );
            }
        }
    ...
    fn paint_current_line(
        ...
        if let Some(breakline) = breakline {
            if let Some(info) = screen_lines.info_for_line(breakline) {
                ...
                cx.fill(
                    &rect,
                    config.color(SnowflakeColor::EDITOR_DEBUG_BREAK_LINE),
                    0.0,
                );
            }
        }
    ...
    fn paint_find(&self, cx: &mut PaintCx, screen_lines: &ScreenLines) {
        ...
        let config = config.get_untracked();
        let line_height = config.editor.line_height() as f64;
        let color = config.color(SnowflakeColor::EDITOR_FOREGROUND);
    ...
    fn paint_sticky_headers(
        ...
        cx.fill(
            &sticky_area_rect,
            config.color(SnowflakeColor::EDITOR_STICKY_HEADER_BACKGROUND),
            0.0,
        );
    ...
    fn paint_scroll_bar(
        ...
        cx.fill(
            &Rect::ZERO
                ...
                .inflate(0.0, 10.0),
            config.color(SnowflakeColor::LAPCE_SCROLL_BAR),
            0.0,
        );
    ...
    fn paint_scope_lines(
        ...
        let config = self.editor.common.config.get_untracked();
        let line_height = config.editor.line_height() as f64;
        let brush = config.color(SnowflakeColor::EDITOR_FOREGROUND);
        ...
        stack((
            ...
            empty().style(move |s| {
                ...
                s.absolute()
                    ...
                    // .border_bottom(1.0)
                    // .border_color(
                    //     config.get_color(SnowflakeColor::LAPCE_BORDER),
                    // )
                    .apply_if(
...
fn editor_gutter_breakpoint_view(
    ...
    breakpoints: RwSignal<BTreeMap<PathBuf, BTreeMap<usize, SnowflakeBreakpoint>>>,
    ...
    container(
        svg(move || config.get().ui_svg(SnowflakeIcons::DEBUG_BREAKPOINT)).style(
            move |s| {
                let config = config.get();
                let size = config.ui.icon_size() as f32 + 2.0;
                s.size(size, size)
                    .color(config.color(SnowflakeColor::DEBUG_BREAKPOINT_HOVER))
...
fn editor_gutter_breakpoints(
        ...
        stack((
            ...
            dyn_stack(
                ...
                move |(line, breakpoint)| {
                    ...
                    container(
                        svg(move || {
                            config.get().ui_svg(SnowflakeIcons::DEBUG_BREAKPOINT)
                        })
                        .style(move |s| {
                            let config = config.get();
                            let size = config.ui.icon_size() as f32 + 2.0;
                            let color = if active {
                                SnowflakeColor::DEBUG_BREAKPOINT
                            } else {
                                SnowflakeColor::EDITOR_DIM
                            };
    ...
    .style(move |s| {
        s.absolute()
            ...
            .background(config.get().color(SnowflakeColor::EDITOR_BACKGROUND))
    })
}
...
fn editor_gutter_code_lens_view(
    ...
    let view = container(svg(move || config.get().ui_svg(SnowflakeIcons::START)).style(
        move |s| {
            let config = config.get();
    ...
    let view = container(
        svg(move || {
            let icon_str = match folding_display_item {
                FoldingDisplayItem::UnfoldStart(_) => SnowflakeIcons::FOLD_DOWN,
                FoldingDisplayItem::Folded(_) => SnowflakeIcons::FOLD,
                FoldingDisplayItem::UnfoldEnd(_) => SnowflakeIcons::FOLD_UP,
            };
...
fn editor_gutter_code_actions(
    ...
    container(
        container(
            svg(move || config.get().ui_svg(SnowflakeIcons::LIGHTBULB)).style(
                move |s| {
                    let config = config.get();
                    let size = config.ui.icon_size() as f32;
                    s.size(size, size)
                        .color(config.color(SnowflakeColor::LAPCE_WARN))
                },
            ),
        )
        .on_click_stop(move |_| {
            ...
            s.padding(4.0)
                ...
                .hover(|s| {
                    s.cursor(CursorStyle::Pointer).background(
                        config.color(SnowflakeColor::PANEL_HOVERED_BACKGROUND),
                    )
                })
                .active(|s| {
                    s.background(
                        ...
                        move |(i, section)| {
                            stack((
                                svg(move || {
                                    config
                                        .get()
                                        .ui_svg(SnowflakeIcons::BREADCRUMB_SEPARATOR)
                                })
                                .style(move |s| {
                                    let config = config.get();
                                    let size = config.ui.icon_size() as f32;
                                    s.apply_if(i == 0, |s| s.hide())
                                        ...
                                        .color(
                                            config.color(
                                                SnowflakeColor::LAPCE_ICON_ACTIVE,
                                            ),
                                        )
                                }),
...
fn search_editor_view(
    ...
    stack((
        ...
        clickable_icon(
            || SnowflakeIcons::SEARCH_CASE_SENSITIVE,
            move || {
        ...
        clickable_icon(
            || SnowflakeIcons::SEARCH_WHOLE_WORD,
            move || {
    ...
    container(
        stack((
            stack((
                clickable_icon(
                    move || {
                        if replace_active.get() {
                            SnowflakeIcons::ITEM_OPENED
                        } else {
                            SnowflakeIcons::ITEM_CLOSED
                        }
                    },
                    move || {
                ...
                clickable_icon(
                    || SnowflakeIcons::SEARCH_BACKWARD,
                    move || {
            ...
            stack((
                ...
                .style(|s| s.padding_left(6.0)),
                clickable_icon(
                    || SnowflakeIcons::SEARCH_REPLACE_ALL,
                    move || {
                        let text = replace_doc
                            .get_untracked()
                            .buffer
    ...
    changes.iter().map(move |change| {
        ...
        let color = match change {
            DiffLines::Left(_range) => {
                Some(config.color(SnowflakeColor::SOURCE_CONTROL_REMOVED))
            }
            DiffLines::Right(_range) => {
                ...
                if modified {
                    Some(config.color(SnowflakeColor::SOURCE_CONTROL_MODIFIED))
                } else {
                    Some(config.color(SnowflakeColor::SOURCE_CONTROL_ADDED))
                }
            }
            _ => None,
        };
