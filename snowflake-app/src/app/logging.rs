...
pub(super) fn logging() -> (Handle<Targets>, Option<WorkerGuard>) {
    ...
    let log_file_filter_targets = filter::Targets::new()
        .with_target("snowflake_app", LevelFilter::DEBUG)
        .with_target("snowflake_proxy", LevelFilter::DEBUG)
        .with_target("snowflake_core", LevelFilter::DEBUG)
...
pub(super) fn panic_hook() {
    std::panic::set_hook(Box::new(move |info| {
        ...
        match info.location() {
            Some(loc) => {
                trace!(
                    target: "snowflake_app::panic_hook",
                    TraceLevel::ERROR,
