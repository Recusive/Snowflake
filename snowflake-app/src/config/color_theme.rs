...
mod tests {
    ...
    use crate::{config::SnowflakeConfig, workspace::SnowflakeWorkspace};

    #[test]
    fn test_resolve() {
        // Mimicking load
        let workspace = SnowflakeWorkspace::default();

        let config = SnowflakeConfig::merge_config(&workspace, None, None);
        let mut snowflake_config: SnowflakeConfig = config.try_deserialize().unwrap();
...
"##;
        ...
        snowflake_config.available_color_themes =
            [("test".to_string(), ("test".to_string(), test_theme_cfg))]
                .into_iter()
                .collect();
        // snowflake_config.available_icon_themes = Some(vec![]);
        snowflake_config.core.color_theme = "test".to_string();
