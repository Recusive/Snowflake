...
use snowflake_rpc::plugin::VoltMetadata;
...
#[test]
fn test_load_volt() {
    let snowflake_proxy_dir = std::env::current_dir()
        .expect("Can't get \"snowflake-proxy\" directory")
        .join("src")
        .join("plugin")
        .join("wasi")
        .join("plugins");
    ...
    let path = snowflake_proxy_dir.join("some-path");
    ...
    if let Ok(volt_metadata) = load_volt(&snowflake_proxy_dir) {
        panic!(
            "Unexpected result from `snowflake_proxy::plugin::wasi::load_volt` function: {volt_metadata:?}"
        );
    }

    // Invalid file (not readable into a string)
    // Making sure the file exists
    let path = snowflake_proxy_dir.join("smiley.png");
    ...
    let parent_path = snowflake_proxy_dir.join("some_author.test-plugin-one");
    ...
    assert_eq!(
        ...
        VoltMetadata {
            ...
            repository: Some("https://github.com/snowflake".to_string()),
            wasm: wasm_path,
    ...
    let parent_path = snowflake_proxy_dir.join("some_author.test-plugin-two");
    ...
    let parent_path = snowflake_proxy_dir.join("some_author.test-plugin-three");
