...
use crate::{
    ...
    workspace::{SnowflakeWorkspace, WorkspaceInfo},
};
...
pub enum SaveEvent {
    App(AppInfo),
    Workspace(SnowflakeWorkspace, WorkspaceInfo),
    RecentWorkspace(SnowflakeWorkspace),
    Doc(DocInfo),
    DisabledVolts(Vec<VoltID>),
    WorkspaceDisabledVolts(Arc<SnowflakeWorkspace>, Vec<VoltID>),
    PanelOrder(PanelOrder),
}

#[derive(Clone)]
pub struct SnowflakeDb {
    folder: PathBuf,
...
impl SnowflakeDb {
    ...
    pub fn recent_workspaces(&self) -> Result<Vec<SnowflakeWorkspace>> {
        let workspaces =
            std::fs::read_to_string(self.folder.join(RECENT_WORKSPACES))?;
        let workspaces: Vec<SnowflakeWorkspace> = serde_json::from_str(&workspaces)?;
        Ok(workspaces)
    }

    pub fn update_recent_workspace(&self, workspace: &SnowflakeWorkspace) -> Result<()> {
    ...
    fn insert_recent_workspace(&self, workspace: SnowflakeWorkspace) -> Result<()> {
...
fn workspace_folder_name(workspace: &SnowflakeWorkspace) -> String {
