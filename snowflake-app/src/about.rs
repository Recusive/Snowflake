...
use snowflake_core::{command::FocusCommand, meta::VERSION, mode::Mode};

use crate::{
    command::{CommandExecuted, CommandKind},
    config::color::SnowflakeColor,
...
impl AboutUri {
    const LAPCE: &'static str = "https://snowflake.dev";
    const GITHUB: &'static str = "https://github.com/no9labs/snowflake";
    const MATRIX: &'static str = "https://matrix.to/#/#snowflake-editor:matrix.org";
        ...
        stack((
            svg(move || (config.get()).logo_svg()).style(move |s| {
                s.size(logo_size, logo_size)
                    .color(config.get().color(SnowflakeColor::EDITOR_FOREGROUND))
            }),
            label(|| "Snowflake".to_string()).style(move |s| {
                s.font_bold()
                    .margin_top(10.0)
                    .color(config.get().color(SnowflakeColor::EDITOR_FOREGROUND))
            }),
            ...
            web_link(
                ...
                move || config.get().color(SnowflakeColor::EDITOR_LINK),
                internal_command,
            )
...
fn exclusive_popup<V: View + 'static>(
    ...
    .style(move |s| {
        ...
        .background(
            config
                .get()
                .color(SnowflakeColor::LAPCE_DROPDOWN_SHADOW)
                .multiply_alpha(0.5),
        )
    })
}
