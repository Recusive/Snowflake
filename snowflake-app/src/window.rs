...
pub struct TabsInfo {
    ...
    pub workspaces: Vec<SnowflakeWorkspace>,
}
...
/// resized using your window manager. Normally Snowflake has only one
...
impl WindowData {
    pub fn new(
        ...
        let cx = Scope::new();
        let config =
            SnowflakeConfig::load(&SnowflakeWorkspace::default(), &[], &extra_plugin_paths);
        ...
        if window_tabs.with_untracked(|window_tabs| window_tabs.is_empty()) {
            let window_tab = Rc::new(WindowTabData::new(
                cx,
                Arc::new(SnowflakeWorkspace::default()),
                common.clone(),
            ));
            window_tabs.update(|window_tabs| {
                window_tabs.push_back((cx.create_rw_signal(0), window_tab));
    ...
    pub fn info(&self) -> WindowInfo {
        let workspaces: Vec<SnowflakeWorkspace> = self
            .window_tabs
            .get_untracked()
            .iter()
